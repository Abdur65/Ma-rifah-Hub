...
import { getAllSections, getAllCategories, checkSlugAvailable } from '@/lib/adminQueries'
...
export function EntryForm({ initial, onSubmit, submitLabel, isSubmitting, onCancel }: Props) {
  const [sectionId, setSectionId] = useState<number | null>(initial?.category_id ? null : null)
  ...
  // Resolve initial section from category
  useEffect(() => {
    if (initial?.category_id && allCategories) {
      const cat = allCategories.find(c => c.id === initial.category_id) as (Category & { section: Section }) | undefined
      if (cat) setSectionId(cat.section_id)
    }
  }, [initial?.category_id, allCategories])

  const filteredCategories = allCategories?.filter(c => c.section_id === sectionId) ?? []
  ...
  useEffect(() => {
    if (!slugManuallyEdited) setSlug(slugify(heading))
  }, [heading, slugManuallyEdited])
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: { class: 'tiptap-content focus:outline-none min-h-[200px] text-slate-800' },
    },
  })
  ...
  }, [sectionId, categoryId, heading, slug, slugError, arabicText, reference, refIsLink, editor, onSubmit])
  ...
  return (
    <form onSubmit={(e) => void handleSubmit(e)} className="space-y-7">
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        <div>
          <label htmlFor="section" className="block text-sm font-medium text-slate-700 mb-1.5">Section</label>
          <select
            id="section"
            value={sectionId ?? ''}
            onChange={e => { setSectionId(Number(e.target.value) || null); setCategoryId(null) }}
      ...
      {/* Heading */}
      <div>
        <label htmlFor="heading" className="block text-sm font-medium text-slate-700 mb-1.5">Heading</label>
        <input
          id="heading"
          type="text"
          value={heading}
      ...
      <div>
        <label htmlFor="slug" className="block text-sm font-medium text-slate-700 mb-1.5">
          Slug <span className="text-slate-400 font-normal text-xs ml-1">— used in the URL</span>
        </label>
        <div className={`flex items-center border rounded-xl overflow-hidden focus-within:border-slate-500 focus-within:ring-2 focus-within:ring-slate-200 transition-colors ${slugError ? 'border-red-400' : 'border-slate-300'}`}>
          <span className="px-4 py-3 bg-slate-50 text-slate-400 text-sm border-r border-slate-300 select-none whitespace-nowrap">
            /{selectedSection?.slug ?? '…'}/{filteredCategories.find(c => c.id === categoryId)?.slug ?? '…'}/
          </span>
          <input
            id="slug"
            type="text"
      ...
      <div style={{ display: isIslamSection ? 'block' : 'none' }}>
        <label htmlFor="arabic-text" className="block text-sm font-medium text-slate-700 mb-1.5">
          Arabic Text <span className="text-slate-400 font-normal text-xs ml-1">— displayed above the English body</span>
        </label>
        <textarea
          id="arabic-text"
          rows={4}
      ...
      <div>
        ...
        <div className={`border rounded-xl overflow-hidden focus-within:border-slate-500 transition-colors ${errors.body ? 'border-red-400' : 'border-slate-300'}`}>
      ...
      {/* Reference */}
      <div>
        <label htmlFor="reference" className="block text-sm font-medium text-slate-700 mb-1.5">Reference / Source</label>
        <input
          id="reference"
          type="text"
          value={reference}
        ...
        {isGKSection && (
          <div className="flex items-center gap-2 mt-2.5">
            ...
            />
            <label htmlFor="ref-is-link" className="text-sm text-slate-600 cursor-pointer">Display as a clickable link</label>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-slate-200">
        <button type="button" onClick={onCancel} className="text-sm text-slate-500 hover:text-slate-800 transition-colors cursor-pointer">
          Cancel
        </button>
        <button
          type="submit"
...
function TipTapToolbar({ editor }: { editor: Editor }) {
  ...
  return (
    <div className="border-b border-slate-200 bg-slate-50 px-3 py-2 flex items-center gap-1 flex-wrap">
      ...
      <div className="w-px h-5 bg-slate-200 mx-1" />
      {btn(editor.isActive('bulletList'),  () => editor.chain().focus().toggleBulletList().run(),  'Bullet list',  <ListIcon />)}
      {btn(editor.isActive('orderedList'), () => editor.chain().focus().toggleOrderedList().run(), 'Ordered list', <OrderedListIcon />)}
      <div className="w-px h-5 bg-slate-200 mx-1" />
      {btn(editor.isActive({ textAlign: 'right' }), () => editor.chain().focus().setTextAlign('right').run(), 'Align right (Arabic)', 'RTL')}
      {btn(editor.isActive({ textAlign: 'left' }),  () => editor.chain().focus().setTextAlign('left').run(),  'Align left',           'LTR')}
    </div>
  )
}
