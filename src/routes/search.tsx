          ...
          <p className="text-sm text-stone-500 mb-6">
            {results?.length === 0 ? (
              ...
              <>
                ...
                for "<span className="text-stone-800">{q}</span>"
              </>
            )}
          </p>
        )}
        ...
        {!isLoading && results && results.length > 0 && (
          <div className="space-y-4">
            {results.map((entry) => {
              ...
              const badgeBg = isIslam
                ? "bg-islam-surface text-islam-primary"
                : "bg-gk-surface text-gk-primary";
              const dotColor = isIslam ? "bg-islam-primary" : "bg-gk-primary";
              const hoverTitle = isIslam
                ? "group-hover:text-islam-primary"
                : "group-hover:text-gk-primary";

              const snippet = DOMPurify.sanitize(entry.body, {
                ALLOWED_TAGS: [],
              }).slice(0, 200);
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    <span
                      className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${badgeBg}`}
                    >
                      <span
                  ...
                  <h3
                    className={`font-[family-name:var(--font-lora)] text-xl font-semibold text-stone-900 mb-2 transition-colors duration-150 ${hoverTitle}`}
                  >
                    {entry.heading}
                  </h3>
                  {snippet && (
