...
export function Navbar({ theme = 'neutral' }: Props) {
  ...
  const bgClass =
    theme === 'islam'   ? 'bg-islam-surface border-islam-border' :
    theme === 'gk'      ? 'bg-gk-surface border-gk-border' :
  ...
  function handleSelect(sectionSlug: string, categorySlug: string, entrySlug: string) {
    setOpen(false)
    setQuery('')
    void navigate({ to: '/$section/$category/$entry', params: { section: sectionSlug, category: categorySlug, entry: entrySlug } })
  }

  return (
    <header className={`border-b px-6 py-4 sticky top-0 z-40 ${bgClass}`}>
      <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
        <Link to="/" className="font-[family-name:var(--font-lora)] text-xl font-semibold text-stone-800 hover:text-stone-600 transition-colors shrink-0">
          Ma'rifah Hub
        </Link>
        ...
        <div ref={wrapRef} className="relative flex-1 max-w-xs">
          <form onSubmit={handleSubmit}>
            <div className="flex items-center bg-white border border-stone-300 rounded-xl px-3 py-2 gap-2 focus-within:border-stone-500 transition-colors">
              ...
              />
              {query && (
                <button type="button" onClick={() => { setQuery(''); setOpen(false) }} className="text-stone-400 hover:text-stone-600 cursor-pointer">
                  <XIcon />
                </button>
              )}
            </div>
            ...
            <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-stone-200 rounded-xl shadow-lg z-50 overflow-hidden">
              {data.slice(0, 5).map(entry => {
                ...
                return (
                  <button
                    key={entry.id}
                    onClick={() => handleSelect(cat.section.slug, cat.slug, entry.slug)}
              ...
              <button
                onClick={() => { setOpen(false); void navigate({ to: '/search', search: { q: query } }) }}
