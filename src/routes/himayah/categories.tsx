...
function CategoriesPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (exists)
      return setFormError(
        "A category with this slug already exists in this section",
      );
    ...
    <main className="max-w-3xl mx-auto px-6 py-10">
      <div className="mb-8">
        ...
        <p className="text-slate-500 text-sm mt-1">
          Manage categories for each section
        </p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden mb-8">
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50">
          <h2 className="text-sm font-semibold text-slate-700">
            {activeSection_?.name ?? "—"} · {sectionCategories.length}{" "}
            {sectionCategories.length === 1 ? "category" : "categories"}
          </h2>
        </div>
                  ...
                  <AlertDialog.Portal>
                    ...
                    <AlertDialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
                      ...
                      <AlertDialog.Description className="text-slate-500 text-sm mb-6">
                        ...
                        " and all its entries will be permanently deleted. This
                        cannot be undone.
