...
export function EntryCard({ entry, theme, linkHref }: Props) {
  const accentColor =
    theme === "islam" ? "text-islam-accent" : "text-gk-accent";
  const surfaceClass =
    theme === "islam"
      ? "bg-islam-surface border-islam-border"
      : "bg-gk-surface border-gk-border";
  const borderClass =
    theme === "islam" ? "border-islam-border" : "border-gk-border";
  const headingColor =
  ...
  return (
    <article
      id={entry.slug}
      className={`bg-white border rounded-2xl overflow-hidden scroll-mt-24 ${borderClass}`}
    >
      <div
        className={`border-b px-8 py-5 flex items-start justify-between gap-4 ${borderClass}`}
      >
        <h2
          className={`font-[family-name:var(--font-lora)] text-2xl font-semibold leading-snug ${headingColor}`}
        >
          {entry.heading}
        </h2>
        {linkHref && (
      ...
      <div className="px-8 py-7 space-y-6">
        {entry.arabic_text && (
          <div className={`border rounded-xl px-6 py-5 ${surfaceClass}`}>
            <p className="arabic text-stone-800 whitespace-pre-wrap">
              {entry.arabic_text}
            </p>
          </div>
        )}
        ...
        {entry.reference && (
          <div
            className={`flex items-start gap-3 pt-4 border-t border-stone-100`}
          >
            ...
            {entry.ref_is_link ? (
              ...
              <span className="font-inter text-sm text-stone-500 italic min-w-0 break-words">
                {entry.reference}
              </span>
            )}
          </div>
