...
function CategoryPage() {
  ...
  if (!section || !category) {
    if (!isLoading && section !== undefined && category !== undefined)
      throw notFound();
    return (
    ...
    <div className={`min-h-screen ${surface}`}>
      ...
      <div className="max-w-3xl mx-auto px-6 pt-5 pb-2">
        <nav className="flex items-center gap-2 text-sm text-stone-500 flex-wrap">
          <Link to="/" className={`hover:${primary} transition-colors`}>
            Home
          </Link>
          <Chevron />
          <Link
      ...
      <div className="max-w-3xl mx-auto px-6 pt-6 pb-10">
        <div className="flex items-center gap-4 mb-2">
          ...
          <div>
            ...
            <h1
              className={`font-[family-name:var(--font-lora)] text-4xl font-bold text-stone-900`}
            >
              {category.name}
            </h1>
          </div>
